...
export const OOTDHistoryDrawer: React.FC<OOTDHistoryDrawerProps> = ({
  ...
  const handleDeleteItem = (id: string, e?: React.MouseEvent) => {
    ...
    setDeleteToast(isEn ? 'Photo removed from Vault!' : 'Đã xóa ảnh khỏi lịch sử Vault!');
  ...
  return (
    ...
    >
      ...
      {/* Spacious Drawer with Smooth Slide-in from the Right */}
      <div
        className={`relative z-10 w-full sm:max-w-xl md:max-w-2xl lg:max-w-3xl bg-white h-full shadow-2xl flex flex-col justify-between overflow-hidden border-l border-gray-100 transition-transform duration-300 ease-out ${
          isVisible ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        
        {/* ========================================================================= */}
        {/* 1. DRAWER HEADER: Separate Language Title (Vault / Kho Lưu Trữ)           */}
        {/* ========================================================================= */}
        <div className="p-5 sm:p-6 pb-4 border-b border-gray-100 space-y-4 shrink-0 bg-white/90 backdrop-blur-md">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-black text-2xl lg:text-3xl text-gray-950 tracking-tight">
                  {isEn ? 'Vault' : 'Kho Lưu Trữ'}
                </h3>
              </div>
            </div>
          ...
          <div className="flex items-center gap-2 pt-1">
            {[
              { id: 'all', label: isEn ? `All (${vaultItems.length})` : `Tất Cả (${vaultItems.length})` },
              {
                id: 'ootd',
          ...
          {filteredItems.length === 0 ? (
            <div className="h-64 flex flex-col items-center justify-center text-center p-6 bg-gray-50 rounded-3xl border border-dashed border-gray-200 space-y-2">
              ...
              <h4 className="text-sm font-black text-gray-700">
                {isEn ? 'No photos in this tab' : 'Chưa có ảnh nào trong mục này'}
              </h4>
              <p className="text-xs text-gray-400">
                {isEn ? 'Try selecting the "All" tab.' : 'Hãy thử bấm vào tab "Tất Cả" để xem toàn bộ ảnh.'}
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {filteredItems.map((item) => {
                ...
                const dateText = isEn ? item.dateEn : item.dateVi;
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between pt-3 mt-3 border-t border-gray-100">
                      ...
                      <button
                        ...
                        title={isEn ? 'Save photo' : 'Lưu ảnh về máy'}
                      >
                        <Download className="w-3.5 h-3.5 text-[#D4FF00]" />
                        <span>{isEn ? 'Save' : 'Lưu Ảnh'}</span>
                      </button>
                      ...
                      <button
                        ...
                        title={isEn ? 'Delete photo' : 'Xóa khỏi lịch sử'}
                      >
          ...
          >
            ...
            <div className="relative w-full h-72 sm:h-80 bg-gray-950">
              <img
                ...
                alt={isEn ? selectedItem.dateEn : selectedItem.dateVi}
              ...
              <div className="absolute bottom-4 left-5 right-5 flex items-center justify-between text-white">
                ...
                <span className="text-xs font-bold text-white/80">
                  {isEn ? selectedItem.dateEn : selectedItem.dateVi}
                </span>
              </div>
            </div>
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5 text-sm font-black text-gray-800">
                  ...
                  <span>{isEn ? selectedItem.dateEn : selectedItem.dateVi}</span>
                </div>
              ...
              <div className="pt-2 flex items-center gap-3">
                ...
                >
                  ...
                  <span>{isEn ? 'Save to Device' : 'Lưu Ảnh Về Máy'}</span>
                </button>
                ...
                >
                  <Trash2 className="w-4 h-4" />
                  <span>{isEn ? 'Delete' : 'Xóa Khỏi Vault'}</span>
                </button>
              </div>
            </div>
          </div>
